...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className="w-full bg-gradient-to-br from-slate-50 to-blue-50 font-sans min-h-screen" ref={containerRef}>
      {/* Header Section */}
      <div className="pb-8">
                ...
                {item.images && item.images.length > 0 && (
                  <div className="mt-6 grid grid-cols-2 gap-4">
                    {item.images.map((imgSrc, imgIndex) => (
                      <div key={imgIndex} className="relative w-full pb-[125%] rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 group">
                        <Image
                          src={imgSrc}
